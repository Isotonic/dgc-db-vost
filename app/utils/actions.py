...
def incident_action(user, action_type, incident, task=None, subtask=None, target_users=None, extra=None):
    if not target_users:
        target_users = []
...
def task_action(user, action_type, task, subtask=None, target_users=None, extra=None):
    ...
    db.session.add(action)
    db.session.commit()
    #emit('activity', {'html': [render_template('action.html', action=action)]}, room=f'{task.incident.deployment_id}-{task.incident.id}')