<template>
  <li class="list-group-item list-group-flush map-card" @click="goTo">
    ...
    <ol v-if="incident.tasks.length || incident.comments.length" class="map-card-actions-list list-unstyled">
      <li class="map-card-actions">
        <i v-if="incident.tasks.length" class="fas fa-tasks" v-tooltip="'Tasks'"></i>
        <span class="map-card-actions-text">{{ taskText }}</span>
        <i v-if="incident.comments.length" class="far fa-comment" v-tooltip="'Comments'"></i>
        <span class="map-card-actions-text">{{ incident.comments.length }}</span>
      </li>
      <ol v-if="incident.assignedTo.length" class="map-card-assigned-to list-unstyled">
